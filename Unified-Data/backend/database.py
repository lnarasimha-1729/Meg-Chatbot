...
The Unified Data product is a pure NL-to-SQL system (NO RAG). All queries run
against one wide table — the Meghalaya Focus Plus farmer payment register —
which lives in Neon as "Meghalaya_Chatbot" (105,813 rows, 22 columns).
...
def _friendly_pg_error(e: Exception) -> str:
    ...
    if "42703" in code or ("column" in msg and "does not exist" in msg):
        ...
        return (f"Column{col} was not found in the table. "
                "Column names with spaces must be double-quoted exactly as in the schema.")
    ...
    if "22007" in code or "invalid input syntax for type date" in msg:
        return ("An invalid date format was used. Note that 'Date of Payment' is stored "
                "as an Excel serial number, not a real date.")
