...
Write path: update L1 immediately → schedule Neon write as background task.
Read path:  L1 hit → return immediately.  L1 miss → read from Neon (≤2s) → populate L1.

If Neon is unavailable the store degrades gracefully to in-memory only.

Table DDL (created on startup via setup()):
  conversation_context (
    id, session_id, question, resolved_question, answer, intent, sql_data, created_at
  )
"""
...
class ContextStore:
    ...
    async def get_context(self, session_id: str) -> list[ConversationTurn]:
        ...
        try:
            turns = await asyncio.wait_for(self._read_from_neon(session_id), timeout=2.0)
