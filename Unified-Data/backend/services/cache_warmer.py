...
# Top queries to pre-warm — match the most common analytical questions.
_WARM_QUERIES = [
    "How many farmers are there in total?",
    "How many farmers are registered in each district?",
    "How many farmers have been paid and how many are pending?",
    "How many records come from each data batch?",
    "What is the total amount paid in West Garo Hills?",
    "How many farmers per bank?",
    "Show new registrations not yet paid",
    "How many farmers have no mobile number recorded?",
    "How many farmers per block in North Garo Hills?",
...
async def warm_cache() -> int:
    ...
    # cannot persist, and firing 30 Gemini calls at startup only saturates the
