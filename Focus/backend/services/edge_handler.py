...
Tuned for the Focus product: a NL-to-SQL assistant over the Meghalaya FOCUS
Producer Group (PG) formation & finance register (focus_pg).
"""
import re, logging
...
_RESPONSES = {
    "greeting": (
        "Khublei! I'm the Focus assistant for the Meghalaya FOCUS Producer Group (PG) "
        "database. I can help you with:\n\n"
        "• PG lookups — by PG name, district, block, bank, or IVCS\n"
        "• Member queries — counts, gender split, age, member bank details\n"
        "• Disbursement — tranche-1 / tranche-2 finance, totals by district or block\n"
        "• Bookkeeper coverage, IVCS linkage, and data-quality gaps\n\n"
        "What would you like to know?"
    ),
    "identity": (
        "I am the **Focus** assistant — a natural-language-to-SQL system built for the "
        "Meghalaya FOCUS Producer Group register (37,354 PGs across 12 districts).\n\n"
        "I can:\n"
        "• Count and list Producer Groups by district, block, bank, or IVCS\n"
        "• Report tranche-1 / tranche-2 disbursement totals and coverage\n"
        "• Analyse members — gender split, age, bank linkage — and bookkeeper coverage\n\n"
        "I'm purpose-built for this dataset — not a general-purpose chatbot. How can I help?"
    ),
    "thanks": (
        "You're welcome! Feel free to ask anything else about the Focus Producer Group records — "
        "PG counts, district/block breakdowns, disbursement, members, and more."
    ),
    "goodbye": (
        "Thank you for using the Focus assistant. Have a great day! "
        "Come back anytime for Producer Group or disbursement queries."
    ),
    "silly": (
        "That's an interesting question, but I'm a **purpose-built assistant** for the Meghalaya "
        "FOCUS Producer Group database — so general topics are outside my scope.\n\n"
    ...
    "profanity": (
        ...
        "FOCUS Producer Group records.\n\n"
        "Let's try again — for example: \"How many PGs have a bookkeeper?\" or "
    ...
    "off_topic": (
        "That's a great question — but it's outside my area! I'm the **Focus** assistant, built "
        "exclusively for the Meghalaya FOCUS Producer Group formation & finance database.\n\n"
        "Could you ask me something about the Producer Group records? For example:\n\n"
        "• \"How many producer groups are there in total?\"\n"
        "• \"Which district has the most Focus PGs?\"\n"
...
def detect_edge_case(question: str) -> dict | None:
    ...
    # MUST run before the data-intent early-exit below, because "focus"/"scheme" are also data words.
    # e.g. "how is Focus different from Focus+", "what is CM Elevate", "what is the join key".
    concept_key = detect_concept_question(ql)
    if concept_key:
        return {"type": "concept", "response": CONCEPT_ANSWERS[concept_key]}

    # ── Early exit: clear PG-data intent → skip all edge checks ──
    _DATA_STRONG = [
        r"\bpg\b", r"\bpgs\b", r"producer\s*group", r"\bgroup", r"\bivcs\b",
        r"\bmember", r"\bbeneficiar", r"\bfarmer", r"\bbookkeeper", r"\bepic\b", r"\bgender\b", r"\bmale\b", r"\bfemale\b",
        r"\bdistrict\b", r"\bblock\b", r"\bvillage\b",
        r"\bdisbursed\b", r"\bdisbursement\b", r"\btranche\b", r"\bfinance\b", r"\bseed\s*capital\b",
        r"\bawaiting\b", r"\bamount\b", r"\bbank\b", r"\baccount\b",
        r"\bmeghalaya\b", r"garo\b", r"khasi\b", r"jaintia\b", r"ri\s*-?\s*bhoi\b",
        r"\bfocus\b", r"\bage\b", r"\bcoverage\b",
    ]
    ...
    # Whitelist: allow questions related to the Producer Group dataset
    data_words = [
        "pg", "producer group", "group", "member", "people",
        "district", "block", "village", "meghalaya",
        "garo", "khasi", "jaintia", "ri-bhoi", "ri bhoi",
        "disbursed", "disbursement", "tranche", "finance", "seed capital", "awaiting",
        "amount", "rupee", "rs", "bank", "account", "ivcs",
        "bookkeeper", "epic", "gender", "male", "female", "age",
        "count", "total", "how many", "show", "list", "find", "search", "lookup",
        "compare", "breakdown", "distribution", "sum", "average", "name", "coverage", "rate",
        ...
        "ratio", "percentage", "percent", "share", "proportion",
        "highest", "lowest", "most", "least", "top", "bottom", "max", "maximum", "min", "minimum",
        "rank", "ranking", "popular", "biggest", "smallest", "largest", "young", "old", "youngest", "oldest",
        "which", "what", "who", "where", "trend", "per ", "each", "group", "by ",
        # cross-scheme vocabulary (Focus / Focus+ / CM Elevate joins)
        "scheme", "schemes", "focus+", "focus plus", "elevate", "loan", "subsidy", "applicant",
