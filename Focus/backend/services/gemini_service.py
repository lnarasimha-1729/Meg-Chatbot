"""
Gemini Service — domain logic layer for the Focus NL-to-SQL engine
==================================================================
...
NL-to-SQL system over the focus_pg Producer Group register.
...
_ALLOWED_TABLES = {"meghalaya_chatbot", "cm_elevate", settings.DATA_TABLE.lower()}
...
async def classify_intent(question: str, context: list[ConversationTurn] = None) -> str:
    ...
    # Hard guard 2: a SINGULAR superlative ("which district/block/PG has the highest/most/lowest/fewest …")
    # wants ONE definitive row. A fresh SQL with LIMIT 1 is unambiguous; reasoning over a prior LIST tends to
    # spill several rows. Route these to SQL unless the user explicitly says "of these / among these / shown".
...
async def generate_sql(question: str, context: list[ConversationTurn] = None) -> tuple[str, float]:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        # questions over focus_pg (data-quality scorecards, multi-CTE district briefs,
        # % coverage with FILTER clauses) generate long SQL that truncated at 512/1024.
        try:
...
def suggest_chart(results: list) -> str | None:
    ...
    if not results:
        return None
    # "total_count" (from COUNT(*) OVER ()) is a window total repeated on every row — it is a
    # headline number for the answer text, NOT a data series. Plotting it would draw a flat bar
    # across every label. Drop it before deciding the chart shape.
    cols = [c for c in results[0].keys() if c != "total_count"]
    if not cols:
        return None
