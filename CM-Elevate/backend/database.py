...
The CM Elevate product is a pure NL-to-SQL system (NO RAG). All queries run
against one wide table — the Meghalaya government scheme disbursement register —
which lives in Neon as "cm_elevate" (2,847 rows, 31 columns).
...
def _friendly_pg_error(e: Exception) -> str:
    ...
    if "42703" in code or ("column" in msg and "does not exist" in msg):
        ...
        return (f"Column{col} was not found in the table. "
                "Note that several disbursement columns are spelled with typos "
                "(loan_disrbusement_1, bank_santioned, refusedy_n) — they must match exactly.")
    ...
    if "22p02" in code or "invalid input syntax for type numeric" in msg or "invalid input syntax for type double" in msg:
        return ("A monetary/amount column could not be parsed as a number. "
                "These columns are stored as text — they must be cast with "
                "NULLIF(col,'')::numeric before arithmetic.")
    ...
    if "22007" in code or "invalid input syntax for type date" in msg:
        return ("An invalid date format was used. Disbursement date columns are stored "
                "as text timestamps (e.g. '2025-03-29 00:00:00') — cast with NULLIF(col,'')::timestamp.")
